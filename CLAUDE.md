...
- **`Changes`**: holds the `replacements` map and everything derived from it. Key methods:
  ...
  - `preview_and_apply` prints the styled preview, then loops on a `y/s/e/n` prompt (or uses `auto_response` from `--yes`/`--save`). `y` applies, `s` serializes via `to_change_list`, `e` calls `edit_changes` (writes the list to a temp file, runs `vim` through `System.run`, reloads it as a new `Changes`, clears `options//grep`, and recurses into `preview_and_apply`). If `vim` is missing or the edited file fails to parse, `edit_changes` returns false and the prompt repeats.
